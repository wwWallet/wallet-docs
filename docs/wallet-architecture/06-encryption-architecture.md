...
- Removed additional encryption of **privateKey**s on **privateData**.
  Following **ADR 02 - Wallet state model**, the **privateKey**s are now
  managed through a map-reduce event-sourcing model, rendering the previous
  encryption layer incompatible with new **privateData** storage architecture.
  Level of security remains the same; The removed layer only served as a
  supplementary safeguard against incidental cleartext exposure.
...
[![Diagram: Wallet encryption architecture](../../static/img/diagrams/wallet-encryption-architecture-v5.svg)](../../static/img/diagrams/wallet-encryption-architecture-v4.svg)
